...
import com.singhand.sd.template.bizmodel.model.BizLogTarget;
import com.singhand.sd.template.bizmodel.repository.BizLogRepository;
import com.singhand.sd.template.bizservice.annotation.resolver.Resolve;
import com.singhand.sd.template.bizservice.annotation.resolver.Resolver;
...
public class BizLoggerAspect {
  ...
  public Object around(ProceedingJoinPoint joinPoint, BizLogger bizLogger) {
    ...
    resolveValuesNeedResponse(joinPoint, proceed, new Resolve[]{bizLogger.module()},
        moduleResolves);
    bizLog.setModule(moduleResolves.get(0));
    ...
    try {
      ...
      new TransactionTemplate(keycloakTransactionManager).executeWithoutResult(transactionStatus ->
          userEntityRepository.findByUsernameAndRealmId(username, keycloakService.getRealm())
              .flatMap(
                  userEntity -> userEntity.getRoles().stream().filter(KeycloakRole::getClientRole)
                      .findFirst())
              .ifPresent(keycloakRole ->
                  bizLog.setUserRole(keycloakRole.getName())));
      ...
      try {
        ...
        new TransactionTemplate(keycloakTransactionManager).executeWithoutResult(
            transactionStatus ->
                userEntityRepository.findByUsernameAndRealmId(username, keycloakService.getRealm())
                    .flatMap(
                        userEntity -> userEntity.getRoles().stream()

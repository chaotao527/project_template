package com.singhand.sd.template.bizbatchservice.converter.html;
...
/**
 * This example shows how to display a spreadsheet in HTML using the classes for spreadsheet
 * display.
 */
...
public final class ToHtml {
  ...
  /**
   * Creates a new examples to HTML for the given workbook.  If the path ends with "{@code .xlsx}"
   * an {@link XSSFWorkbook} will be used; otherwise this will use an {@link HSSFWorkbook}.
   *
  ...
  /**
   * Creates a new examples to HTML for the given workbook.  This attempts to detect whether the
   * input is XML (so it should create an {@link XSSFWorkbook} or not (so it should create an
   * {@link HSSFWorkbook}).
   *
   ...
   */
  private int widthToPixels(final double widthUnits) {
